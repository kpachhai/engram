...
Both models include the optional ``vault`` and ``legacy_id`` fields. Single-
vault deployments default ``vault="default"`` and leave ``legacy_id`` unset
unless populated by the migration command; carrying these in v1.0 outputs is
the forward-compat commitment.
"""
...
class Thought(BaseModel):
    ...
    #: GPG primary-key fingerprint (40 hex; canonical form) of the capturing
    #: user when the thought lands in a team-write vault. None for
    #: personal-vault captures.
