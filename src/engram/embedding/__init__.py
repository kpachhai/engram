...
uses for vector generation. :class:`FastEmbedProvider` is the Phase 1
implementation backed by FastEmbed (``BAAI/bge-small-en-v1.5`` by default,
384-dim).

Per-file SHA-256 model verification is structured but Phase 1 ships with
empty hash manifests. ``engram doctor --download-model`` prints the
computed hashes for the maintainer to pin into ``model_hashes.py``;
populating real hashes is a Phase 1.1 follow-up. Until then, the FastEmbed
default trust anchor (HuggingFace HTTPS) is the integrity boundary.
"""
