...
| **P4-H6** | `default_user` (free-form string) is too weak for cross-trust attribution - two members can collide on `alice` from different `git config user.email` values | Step 3 + 6 - Phase 4 sender id is the GPG signing key fingerprint (short-hash 16 chars). The `members.yaml` enrollment file maps fingerprint → display name. Refusal: capture into team-write vault refuses with `team_member_not_enrolled` if the local key isn't enrolled |
